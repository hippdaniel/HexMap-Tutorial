...
public class HexGridChunk : MonoBehaviour
{
    ...
    private void TriangulateRoadAdjacentToRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        else if(cell.IncomingRiver == cell.OutgoingRiver.Opposite())
        {
            ...
            if (previousHasRiver)
            {
                ...
                corner = HexMetrics.GetSolidCorner(direction, 2);
            }
            else
            {
                if (!hasRoadThroughEdge && !cell.HasRoadThroughEdge(direction.Previous())) return;
                corner = HexMetrics.GetSolidCorner(direction, 1);
            }
        ...
        {
            roadCenter -= HexMetrics.GetCorner(cell.IncomingRiver, 2) * 0.2f;
        }
        else if (cell.IncomingRiver == cell.OutgoingRiver.Next())
        {
            roadCenter -= HexMetrics.GetCorner(cell.IncomingRiver, 1) * 0.2f;
        }
    ...
    private void TriangulateWaterShore(HexDirection direction, HexCell cell, HexCell neighbor, Vector3 center)
    {
        ...
        center2.y = center.y;
        EdgeVertices e2 = new EdgeVertices(
            center2 + HexMetrics.GetSolidCorner(direction.Opposite(), 2),
            center2 + HexMetrics.GetSolidCorner(direction.Opposite(), 1));
        ...
        if (nextNeighbor != null)
        {
            Vector3 v3 = nextNeighbor.Position + (nextNeighbor.IsUnderwater
                ...
                : HexMetrics.GetSolidCorner(direction.Previous(), 1));
            v3.y = center.y;
    ...
    private void TriangulateEstuary(EdgeVertices e1, EdgeVertices e2, bool incomingRiver){
        ...
        estuaries.AddTriangleUV(new Vector2(0f, 1f), new Vector2(1f, 1f), new Vector2(1f, 1f));
        estuaries.AddQuadUV(new Vector2(0f, 0f), new Vector2(0f, 0f), new Vector2(1f, 1f), new Vector2(0f, 1f));

        if (incomingRiver)
        {
            estuaries.AddQuadUV2(new Vector2(1.5f, 1f), new Vector2(0.7f, 1.15f), new Vector2(1f, 0.8f),
                new Vector2(0.5f, 1.1f));
            estuaries.AddTriangleUV2(new Vector2(0.5f, 1.1f), new Vector2(1f, 0.8f), new Vector2(0f, 0.8f));
            estuaries.AddQuadUV2(new Vector2(0.5f, 1.1f), new Vector2(0.3f, 1.15f), new Vector2(0f, 0.8f),
                new Vector2(-0.5f, 1f));
        }
        else
        {
            estuaries.AddQuadUV2(new Vector2(-0.5f, -0.2f), new Vector2(0.3f, -0.35f), new Vector2(0f, 0f),
                new Vector2(0.5f, -0.3f));
            estuaries.AddTriangleUV2(new Vector2(0.5f, -0.3f), new Vector2(0f, 0f), new Vector2(1f, 0f));
            estuaries.AddQuadUV2(new Vector2(0.5f, -0.3f), new Vector2(0.7f, -0.35f), new Vector2(1f, 0f),
